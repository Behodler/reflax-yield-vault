 ...
 * @notice Integration tests for SurplusTracker with AutoPoolYieldStrategy
...
contract SurplusTrackerIntegrationTest is Test {
    ...
    AutoPoolYieldStrategy public autoPoolVault;
    AutoPoolYieldStrategy public secondVault; // For multi-vault tests
    ...
    MockAutoPool public autoPool2; // For second vault
    ...
    function setUp() public {
        ...
        autoPool = new MockAutoPool("AutoPool", "autoPool", address(underlyingToken), address(mainRewarder));

        // Deploy mock external dependencies for second vault
        mainRewarder2 = new MockMainRewarder(address(tokeToken));
        autoPool2 = new MockAutoPool("AutoPool2", "autoPool2", address(underlyingToken), address(mainRewarder2));

        // Deploy first AutoPoolYieldStrategy
        autoPoolVault = new AutoPoolYieldStrategy(
            owner, address(underlyingToken), address(tokeToken), address(autoPool), address(mainRewarder)
        );
        autoPoolVault.setClient(client1, true);
        autoPoolVault.setClient(client2, true);

        // Deploy second AutoPoolYieldStrategy for multi-vault tests
        secondVault = new AutoPoolYieldStrategy(
            owner, address(underlyingToken), address(tokeToken), address(autoPool2), address(mainRewarder2)
        );
        secondVault.setClient(client1, true);
        secondVault.setClient(client2, true);

        // Mint tokens to clients and external vaults
        underlyingToken.mint(client1, 10000e18);
        underlyingToken.mint(client2, 10000e18);
        underlyingToken.mint(address(autoPool), 10000e18);
        underlyingToken.mint(address(autoPool2), 10000e18);
    }
    ...
    function testBasicSurplusCalculation() public {
        ...
        underlyingToken.approve(address(autoPoolVault), 1000e18);
        autoPoolVault.deposit(address(underlyingToken), 1000e18, client1);
        vm.stopPrank();

        // Simulate yield accrual (10% = 100 DOLA)
        underlyingToken.mint(address(autoPool), 100e18); // Mint tokens for yield payout
        autoPool.simulateYield(100e18); // Update internal accounting

        // Calculate surplus (internal balance = 900)
        uint256 surplus = tracker.getSurplus(address(autoPoolVault), address(underlyingToken), client1, 900e18);
    ...
    function testNoSurplusWhenBalancesMatch() public {
        ...
        uint256 surplus = tracker.getSurplus(address(autoPoolVault), address(underlyingToken), client1, 1000e18);
    ...
    function testAutoDolaVaultSurplusWithYield() public {
        ...
        assertEq(autoPoolVault.principalOf(address(underlyingToken), client1), 1000e18);

        // Simulate yield accrual in autoPool (5% yield = 50 DOLA)
        underlyingToken.mint(address(autoPool), 50e18); // Mint tokens for yield payout
        autoPool.simulateYield(50e18); // Update internal accounting

        // totalBalanceOf should now include yield
        uint256 totalBalance = autoPoolVault.totalBalanceOf(address(underlyingToken), client1);
    ...
    function testAutoDolaVaultSurplusNoYield() public {
        ...
        assertEq(autoPoolVault.principalOf(address(underlyingToken), client1), 1000e18);
        assertApproxEqAbs(
            autoPoolVault.totalBalanceOf(address(underlyingToken), client1),
            1000e18,
            1,
    ...
    function testAutoDolaVaultSurplusMultipleClients() public {
        ...
        vm.startPrank(client1);
        underlyingToken.approve(address(autoPoolVault), 2000e18);
        autoPoolVault.deposit(address(underlyingToken), 2000e18, client1);
        vm.stopPrank();

        // Client2 deposits 1000 DOLA
        vm.startPrank(client2);
        underlyingToken.approve(address(autoPoolVault), 1000e18);
        autoPoolVault.deposit(address(underlyingToken), 1000e18, client2);
        vm.stopPrank();

        // Simulate yield accrual (10% yield on 3000 total = 300 DOLA)
        underlyingToken.mint(address(autoPool), 300e18); // Mint tokens for yield payout
        autoPool.simulateYield(300e18); // Update internal accounting

        // Calculate surplus for client1 (should get 2/3 of yield = 200 DOLA)
        uint256 surplus1 = tracker.getSurplus(address(autoPoolVault), address(underlyingToken), client1, 2000e18);

        // Calculate surplus for client2 (should get 1/3 of yield = 100 DOLA)
        uint256 surplus2 = tracker.getSurplus(address(autoPoolVault), address(underlyingToken), client2, 1000e18);
    ...
    function testSurplusTrackerWorksWithMultipleVaults() public {
        ...
        vm.startPrank(client1);
        underlyingToken.approve(address(secondVault), 2000e18);
        secondVault.deposit(address(underlyingToken), 2000e18, client1);
        vm.stopPrank();

        // Accrue different yield in second vault
        underlyingToken.mint(address(autoPool2), 200e18); // Mint tokens for yield payout
        autoPool2.simulateYield(200e18); // Update internal accounting

        uint256 secondVaultSurplus = tracker.getSurplus(address(secondVault), address(underlyingToken), client1, 2000e18);
    ...
    function testRealisticBehodlerScenario() public {
        ...
        vm.startPrank(client1);
        underlyingToken.approve(address(autoPoolVault), 10000e18);
        autoPoolVault.deposit(address(underlyingToken), 10000e18, client1);
        vm.stopPrank();

        // Verify principal matches deposit
        assertEq(autoPoolVault.principalOf(address(underlyingToken), client1), 10000e18);

        // AutoDola vault accrues 5% yield (500 DOLA)
        underlyingToken.mint(address(autoPool), 500e18); // Mint tokens for yield payout
        autoPool.simulateYield(500e18); // Update internal accounting
        ...
        uint256 surplus = tracker.getSurplus(address(autoPoolVault), address(underlyingToken), client1, 10000e18);
    ...
    function testSurplusAfterPartialWithdrawal() public {
        ...
        // Accrue yield
        underlyingToken.mint(address(autoPool), 1000e18); // Mint tokens for yield payout
        autoPool.simulateYield(1000e18); // Update internal accounting

        // Initial surplus calculation (internal = 9000, vault has principal + yield)
        uint256 surplusBefore = tracker.getSurplus(address(autoPoolVault), address(underlyingToken), client1, 9000e18);
        ...
        vm.prank(client1);
        autoPoolVault.withdraw(address(underlyingToken), 2000e18, client1);
        ...
        uint256 surplusAfter = tracker.getSurplus(address(autoPoolVault), address(underlyingToken), client1, 7000e18);
    ...
    function testMultipleYieldAccruals() public {
        ...
        uint256 surplus1 = tracker.getSurplus(address(autoPoolVault), address(underlyingToken), client1, 1000e18);
        ...
        uint256 surplus2 = tracker.getSurplus(address(autoPoolVault), address(underlyingToken), client1, 1000e18);
        ...
        uint256 surplus3 = tracker.getSurplus(address(autoPoolVault), address(underlyingToken), client1, 1000e18);
